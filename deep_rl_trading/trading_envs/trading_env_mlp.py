...
class TradingEnvMLP(gym.Env):
    """Custom Environment for Futures Trading"""
    metadata = {'render.modes': ['human']}

    def __init__(
                    self, nifty_df, save_file, action_dim, observation_dim,
                    margin_pct, max_num_lots, lot_size, num_lots_held, lambda_,
                    model_name
                ):
        super(TradingEnvMLP, self).__init__()
    ...
    def step(self, action):
        ...
        prev_price = self.nifty_df.iloc[self.idx]["Close_1"]

        self.idx += 1
        current_price = self.nifty_df.iloc[self.idx]["Close_1"]
        ...
        market_values = np.array(self.nifty_df.iloc[self.idx][7:-3].values)
        action = np.array([action])
        next_state = np.concatenate((market_values, action), axis=None)
        next_state = next_state.astype("float64")
