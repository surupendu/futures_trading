...
class CNN_Price_News_Senti_Layer(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Dict, feature_dim):
        super(CNN_Price_News_Senti_Layer, self).__init__(observation_space, feature_dim)
        self.conv_1 = nn.Conv1d(in_channels=768, out_channels=100, kernel_size=3)
        self.conv_2 = nn.Conv1d(in_channels=15, out_channels=15, kernel_size=3)

        self.linear_1 = nn.Linear(100, 100)
        self.linear_2 = nn.Linear(100, 1)
        self.sigmoid = nn.Sigmoid()

        self.linear_3 = nn.Linear(15, 15)
        self.linear_4 = nn.Linear(17, feature_dim)
